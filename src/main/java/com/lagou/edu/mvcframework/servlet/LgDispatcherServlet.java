        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if (method.isAnnotationPresent(LagouSecurity.class)) {
                    String name = method.getAnnotation(LagouSecurity.class).value();
                    handler = new Handler(entry.getValue(),method, Pattern.compile(url), name.split(","));
                } else {
                    handler = new Handler(entry.getValue(),method, Pattern.compile(url));
                }
...
//        method.invoke() //
        ...
        // 判断传参用户是否有权限
        List<String> userNames = Arrays.asList(handler.getAllowUserNames());

        String reqUserName = StringUtils.join(parameterMap.get("username"), ",");
        if (!userNames.contains(reqUserName)) {
